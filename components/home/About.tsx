...
export default function About() {
  return (
    <section
      id="about"
      className="about w-full h-full flex flex-col items-center justify-center"
    >
      <div className="container mx-auto h-full w-[95%] lg:w-[75%] flex flex-col items-center justify-center gap-6 md:gap-10 lg:gap-16  py-10 px-10 rounded-3xl bg-black/20 shadow-black shadow-[0_0_60px_15px_rgba(0,0,0,0.3)]">
        ...
        <h3 className="border-l-4 border-teal-500 px-2">Mes compétences</h3>
        <IconContext.Provider value={{ color: "white" }}>
          <ul className="grid grid-cols-4 md:grid-cols-8 lg:grid-cols-8 gap-4">
            {Skills.map((Skill, index) => (
              <li
                key={index}
                className="p-2 rounded-xl bg-zinc-900/50 hover:-translate-y-2 transition-all ease-out duration-300"
              >
                <Skill.icon
                  title={Skill.title}
